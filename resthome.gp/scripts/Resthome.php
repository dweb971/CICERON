...
class Resthome
{
    ...
    public function addR(array $data)
    {
        ...
        $reqI= "INSERT INTO recette (idcategorie, nom recette,ingredient,instruction,image, nbr_personne, dateAdd, dateUpdate) VALUES('".$this->get_catR()."', '".$this->get_nomR()."', '".$this->get_ingR()."' , '".$this->get_instR()."', '".$this->get_imgR()."', '".$this->get_persR()."', '".$dates."', '".$dates."')";
    ...
    * qui se trouve avant l'appel à la fonction header() */
    ...
    public function login(array $data)
    {
        ...
        if($dbh->rowCount() === 1)
        {
            // trouve un résultat // verification password
            foreach($dbh as $row)
            {
                if (password_verify($this->get_password(), $row["mot_passe"]))
                {
                    // redirection dashboard et déclaration sessions
